"""Deterministic, no-model verification for the V3 real-user replay package."""
...
PACKAGE_ID = "real_user_multiturn_replay_20260902_v3"
...
def dummy_outputs(conversations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    schedule_builder = load_module("build_replay_schedule.py", "rumr_v3_schedule_for_dummy")
        ...
        for turn_index, turn in enumerate(conversation["turns"]):
            for arm in ARMS:
                ...
                trace = {
                    ...
                    "controlledWorldHash": sha_text("controlled-world|frozen-v3"),
                    "modelConfigurationHash": sha_text("model-configuration|frozen-v3"),
                    "toolConfigurationHash": sha_text("tool-configuration|frozen-v3"),
                    "budgetConfigurationHash": sha_text("budget-configuration|frozen-v3"),
                    "policyConfigurationHash": sha_text("policy-configuration|frozen-v3"),
                ...
                row = {
                        "schemaVersion": "real-user-multiturn-paired-output-v3",
                        "conversationId": conversation["conversationId"],
                        "turnId": turn["turnId"],
...
def _verify_legacy_unchanged(path: Path, expected: dict[str, str], label: str) -> int:
    ...
    assert actual == expected, f"legacy {label} package changed after v3 repair began"
    return len(actual)
...
def verify_all() -> dict[str, Any]:
    ...
    assert replay["schemaVersion"] == "real-user-multiturn-ab-replay-contract-v3"
    ...
    builder = load_module("blind_packet_builder.py", "rumr_v3_blind_builder")
    ...
    schedule_builder = load_module("build_replay_schedule.py", "rumr_v3_schedule_verify")
    ...
    judge_example = {
        "schemaVersion": "real-user-multiturn-ai-judge-response-v3",
    ...
    return {
        ...
        "jsonInstancesValidated": 8 + 8 + 42 + 1,
