"""Audit or run the public-dev-only used-phone two-stage ranking v3."""
...
def main() -> int:
    ...
    from agent.evaluation.used_phone_two_stage_ranking_runner_v3 import (
        PublicRunnerError,
        audit_public_inputs,
        run_public_agent,
    )
    ...
    print(json.dumps({
        key: manifest[key]
        for key in (
            "failedCaseIds", "modelCallCount", "businessWriteNetworkUsed",
            "hiddenArtifactsRead", "prediction",
        )
