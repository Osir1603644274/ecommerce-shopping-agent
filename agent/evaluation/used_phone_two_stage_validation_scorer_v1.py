"""Prediction-first scorer for the one-shot blind Validation10 decision set.

Run-owned bytes and all public identities are authenticated before this module
opens the evaluator-owned source.  The source reader skips the Dev block as
opaque bytes, parses exactly the V01-V10 block, and stops before the Test block.
No prediction or report field contains judgment item identifiers.
"""
...
SCHEMA_VERSION = "used-phone-two-stage-ranking-validation10-score-v1"
EVALUATOR_IDENTITY_VERSION = "validation10-prediction-first-v1"
PROTOCOL_VERSION = (
    "used-phone-public-production-agent-runner-v3-two-stage-ranking-validation10"
)
VALIDATION_CASE_IDS = tuple(f"UPV2-RK-V{index:02d}" for index in range(1, 11))
PREREGISTRATION_FILENAME = "validation10_preregistration_v1.json"
PREREGISTRATION_SHA256 = (
    "8dc8d66ed9ac7b2b7d79fc42652f2c3701ecc0f22f48cf72e680d752bc855288"
)
HIDDEN_FILENAME = "judgments_hidden.jsonl"
HIDDEN_SOURCE_BYTE_COUNT = 10_947_321
HIDDEN_SOURCE_ROW_COUNT = 7_560
VALIDATION_BLOCK_ROW_COUNT = 2_520
VALIDATION_BLOCK_BYTE_COUNT = 3_660_027
VALIDATION_BLOCK_SHA256 = (
    "48e2785b1ac1462392f1da860d8484429ef7ecda0e124e6f179d88c253852186"
)
ROWS_PER_CASE = 252
...
RUNNER_FILES = {
    ...
    "agent/evaluation/used_phone_two_stage_validation_runner_v1.py": (
        REPO_ROOT / "agent" / "evaluation" / "used_phone_two_stage_validation_runner_v1.py"
    ),
    ...
    "agent/scripts/run_used_phone_two_stage_validation_v1.py": (
        REPO_ROOT / "agent" / "scripts" / "run_used_phone_two_stage_validation_v1.py"
    ),
}
EVALUATOR_FILES = {
    "agent/evaluation/used_phone_two_stage_validation_scorer_v1.py": Path(__file__).resolve(),
    ...
    "agent/evaluation/used_phone_two_stage_ranking_metric_audit_v1.py": (
        REPO_ROOT / "agent" / "evaluation"
        / "used_phone_two_stage_ranking_metric_audit_v1.py"
    ),
    "agent/evaluation/schemas/used_phone_two_stage_ranking_prediction_v3.schema.json": PREDICTION_SCHEMA_PATH,
    "agent/scripts/score_used_phone_two_stage_validation_v1.py": (
        REPO_ROOT / "agent" / "scripts" / "score_used_phone_two_stage_validation_v1.py"
    ),
}
...
@contextmanager
def _validation_auth_identity():
    with (
        patch.object(_base, "PROTOCOL_VERSION", PROTOCOL_VERSION),
        patch.object(_base, "PUBLIC_DEV_CASE_IDS", VALIDATION_CASE_IDS),
        patch.object(_base, "RUNNER_FILES", RUNNER_FILES),
        patch.object(_base, "_validate_run_identity", _validate_validation_run_identity),
    ):
        yield


def _validate_validation_run_identity(
    *, manifest: Mapping[str, Any], run_contract: Mapping[str, Any],
    ...
    if manifest.get("selectedSplits") != ["validation"]:
        raise ValueError("Validation10 scorer accepts only the validation split")
    if manifest.get("selectedCaseIds") != list(VALIDATION_CASE_IDS):
        raise ValueError("Validation10 case identity/order mismatch")
    execution = manifest.get("execution")
    if (
        not isinstance(execution, dict)
        or list(execution) != list(VALIDATION_CASE_IDS)
        or any(type(value) is not bool for value in execution.values())
    ):
        raise ValueError("Validation10 execution identity mismatch")
    ...
    if (
        ...
        raise ValueError("Validation10 requires ten terminal successes")
    prediction = manifest.get("prediction")
    if (
        not isinstance(prediction, dict)
        or prediction.get("path") != "predictions.jsonl"
        or prediction.get("rowCount") != len(VALIDATION_CASE_IDS)
    ...
    if (
        ...
        raise ValueError("Validation10 run exceeded hidden/write/no-model boundary")
    expected = {
        ...
        "selectedCaseIds": list(VALIDATION_CASE_IDS),
...
def _authenticate_prediction_before_labels(
    *, predictions_path: Path, manifest_path: Path,
    ...
    with _validation_auth_identity():
...
def _load_preregistration(path: Path, attempt_id: str) -> dict[str, Any]:
    ...
    if (
        resolved.parent != PREREGISTRATION_DIR.resolve()
        or resolved.name != PREREGISTRATION_FILENAME
    ):
        raise ValueError("Validation10 preregistration path mismatch")
    payload = resolved.read_bytes()
    if hashlib.sha256(payload).hexdigest() != PREREGISTRATION_SHA256:
        raise ValueError("Validation10 preregistration SHA mismatch")
    value = json.loads(payload)
    if payload != canonical_bytes(value):
        raise ValueError("Validation10 preregistration must be canonical JSON")
    if (
        value.get("schemaVersion")
        != "used-phone-two-stage-ranking-validation10-preregistration-v1"
        or value.get("split") != "validation"
        or value.get("caseIds") != list(VALIDATION_CASE_IDS)
        or value.get("frozenBeforeValidationPrediction") is not True
        or attempt_id not in value.get("authorizedRunDirectories", [])
    ):
        raise ValueError("Validation10 preregistration semantic mismatch")
    return value


def _load_validation_judgments(path: Path) -> dict[str, dict[str, Any]]:
    resolved = path.resolve()
    if resolved.parent != EVALUATOR_ASSET_DIR.resolve() or resolved.name != HIDDEN_FILENAME:
        raise ValueError("Validation10 judgments must use the pinned evaluator asset")
    if resolved.stat().st_size != HIDDEN_SOURCE_BYTE_COUNT:
        raise ValueError("hidden judgment source byte count mismatch")
    judgments: dict[str, dict[str, Any]] = {
        case_id: {} for case_id in VALIDATION_CASE_IDS
    }
    validation_hasher = hashlib.sha256()
    validation_bytes = 0
    observed_order: list[str] = []
    with resolved.open("rb", buffering=0) as stream:
        for line_number in range(1, VALIDATION_BLOCK_ROW_COUNT * 2 + 1):
            raw = stream.readline()
            if not raw:
                raise ValueError("hidden judgment source ended before Validation10 block")
            if line_number <= VALIDATION_BLOCK_ROW_COUNT:
                continue
            validation_hasher.update(raw)
            validation_bytes += len(raw)
            if not raw.strip() or not raw.endswith(b"\n"):
                raise ValueError("invalid Validation10 judgment JSONL boundary")
            try:
                row = json.loads(raw)
            except (UnicodeDecodeError, json.JSONDecodeError) as exc:
                raise ValueError("invalid Validation10 judgment JSON") from exc
            if not isinstance(row, dict) or raw != canonical_bytes(row):
                raise ValueError("non-canonical Validation10 judgment row")
            case_id = row.get("caseId")
            item_id = row.get("itemId")
            if (
                case_id not in judgments
                ...
                raise ValueError("Validation10 judgment identity mismatch")
            if not observed_order or observed_order[-1] != case_id:
                if case_id in observed_order:
                    raise ValueError("Validation10 case block is non-contiguous")
                observed_order.append(case_id)
            if item_id in judgments[case_id]:
                raise ValueError("duplicate Validation10 judgment")
            judgments[case_id][item_id] = row
    if (
        validation_bytes != VALIDATION_BLOCK_BYTE_COUNT
        or validation_hasher.hexdigest() != VALIDATION_BLOCK_SHA256
    ):
        raise ValueError("Validation10 judgment block byte identity mismatch")
    if (
        tuple(observed_order) != VALIDATION_CASE_IDS
        or any(len(rows) != ROWS_PER_CASE for rows in judgments.values())
    ):
        raise ValueError("Validation10 judgment case/cardinality mismatch")
    item_universe = set(judgments[VALIDATION_CASE_IDS[0]])
    if len(item_universe) != ROWS_PER_CASE or any(
        set(rows) != item_universe for rows in judgments.values()
    ):
        raise ValueError("Validation10 judgment item universe mismatch")
    return judgments
...
def _normalized_diagnostics(
    prediction: Mapping[str, Any], judgments: Mapping[str, Mapping[str, Any]],
) -> dict[str, Any]:
    row = _case_diagnostics(prediction, judgments)
    pool_hits = row["candidatePoolRelevantHitCount"]
    final_hits = row["finalRelevantHitCount"]
...
def score_validation10(
    *, predictions_path: Path, manifest_path: Path,
    ...
    prereg = _load_preregistration(validation_preregistration_path, attempt_id)
    judgments = _load_validation_judgments(validation_judgments_path)
    per_case = [
        _normalized_diagnostics(row, judgments[row["caseId"]])
    ...
    metrics = {
        "candidateConditionedRetentionCeilingUtilizationAt20": _mean(
            row["candidateConditionedRetentionCeilingUtilizationAt20"]
            for row in per_case
        ),
        ...
        "top10HardViolationCount": sum(
            row["top10HardViolationCount"] for row in per_case
        ),
        "top10HardViolationDenominator": sum(
            row["top10HardViolationDenominator"] for row in per_case
        ),
    ...
    return {
        "evaluatorIdentity": {
            ...
            "validationBlockByteCount": VALIDATION_BLOCK_BYTE_COUNT,
            "validationBlockRowCount": VALIDATION_BLOCK_ROW_COUNT,
            "validationBlockSha256": VALIDATION_BLOCK_SHA256,
        },
        "inputs": {
            "manifestSha256": manifest_sha,
            "predictionSha256": prediction_sha,
            "runContractSha256": contract_sha,
            "validationPreregistrationSha256": PREREGISTRATION_SHA256,
        },
