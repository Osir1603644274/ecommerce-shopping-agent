...
def _load_schedule_builder():
    ...
    spec = importlib.util.spec_from_file_location("rumr_v3_schedule_builder", path)
...
def _reject_visible_arm_leakage(value: Any, forbidden_identities: set[str], path: str = "$") -> None:
    ...
    if isinstance(value, str):
        ...
        if any(label in normalized for label in FORBIDDEN_VISIBLE_KEYS if len(label) >= 8):
...
def build_packets(
    ...
    for conversation in conversations:
        for turn_index, turn in enumerate(conversation["turns"]):
            ...
            common = {
                "schemaVersion": "real-user-multiturn-ai-blind-item-v3",
...
def main() -> int:
    ...
    sealed.write_text(
        json.dumps(
            {
                "schemaVersion": "real-user-multiturn-ai-blind-mapping-v3",
                "seed": args.seed,
                "items": built["mapping"],
            },
            ensure_ascii=False,
    ...
    receipt = {
        "schemaVersion": "real-user-multiturn-ai-blind-package-receipt-v3",
