"""One-shot held-out Validation20 identity over the production runner kernel."""
...
PREDICTION_SCHEMA_VERSION = "used-phone-natural-guide-validation20-prediction-v1"
...
EXPECTED_FAMILY_COUNTS = {
    ...
    "comparison": 3,
    "multi_turn_delta": 5,
    "natural_constraints": 7,
    "substitute": 1,
    "unsupported_or_no_match": 2,
}
EXPECTED_CASE_IDS = tuple(f"UPNG-V{index:02d}" for index in range(1, 21))
...
def _allowed() -> frozenset[Path]:
    ...
    return frozenset({
        ...
        (root / "agent" / "scripts" / "run_used_phone_natural_guide_validation20_v1.py").resolve(),
...
def _hashes() -> dict[str, str]:
    ...
    paths = {
        ...
        "agent/evaluation/used_phone_natural_guide_validation20_runner_v1.py": Path(__file__).resolve(),
        "agent/evaluation/schemas/used_phone_natural_guide_validation20_prediction_v1.schema.json": SCHEMA_PATH.resolve(),
        "agent/scripts/run_used_phone_natural_guide_validation20_v1.py": (root / "agent" / "scripts" / "run_used_phone_natural_guide_validation20_v1.py").resolve(),
    }
...
def _identity():
    if not _LOCK.acquire(blocking=False):
        raise PublicRunnerError("Validation20 identity is already active")
    original = {name: getattr(_kernel, name) for name in _FIELDS}
    original_allowed, original_hashes = _kernel._allowed_runtime_repo_files, _kernel._runner_code_hashes
    try:
        ...
        _kernel._allowed_runtime_repo_files, _kernel._runner_code_hashes = original_allowed, original_hashes
...
async def run_public_agent(
    *, public_cases_path: Path, public_catalog_path: Path, run_dir: Path,
    ...
    if allow_sealed_test or resume:
        raise PublicRunnerError("Validation20 is public-input one-shot and requires a fresh run")
    selected_splits = list(splits or ["validation"])
    selected_ids = list(case_ids) if case_ids is not None else list(EXPECTED_CASE_IDS)
    if selected_splits != ["validation"] or selected_ids != list(EXPECTED_CASE_IDS):
        raise PublicRunnerError("Validation20 requires the complete ordered validation set")
    with _identity():
        return await _kernel.run_public_agent(
            public_cases_path=public_cases_path, public_catalog_path=public_catalog_path,
            run_dir=run_dir, splits=selected_splits, case_ids=selected_ids,
            allow_sealed_test=False, resume=False, timeout_seconds=timeout_seconds,
        )
