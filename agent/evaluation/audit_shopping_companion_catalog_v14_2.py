"""Audit hierarchical Shopping Companion catalog identity before V14 quality.
...
def category_id(value: object) -> str:
    """Retain the leaf helper for diagnostics and compatibility."""
    return category_ids(value)[-1]


def category_ids(value: object) -> tuple[str, ...]:
    """Represent a product as a member of every public breadcrumb category."""
    parts = [part.strip() for part in str(value).split(" - ") if part.strip()]
    raw_parts = parts if parts else [str(value)]
    result: list[str] = []
    for part in raw_parts:
        normalized = slug(part, limit=64)
...
def product_tuples(product: dict[str, Any], shapes: Counter[str]) -> set[TupleKey]:
    ...
    if type(category) is not str or not category.strip():
        ...
        return set()
    normalized_categories = category_ids(category)
    ...
    for source in sources:
        for raw_key, raw_values in source.items():
            ...
            for value in values:
                normalized_value = slug(value, limit=128)
                for normalized_category in normalized_categories:
                    result.add((normalized_category, key, normalized_value))
    return result
...
def audit_catalog(
    ...
    *,
    ...
    report = {
        "schemaVersion": "shopping-memory-v14-catalog-structure-audit-v1.2",
        "decision": decision,
        ...
        "categoryIdentity": "all exact ' - ' breadcrumb segments, independently normalized",
