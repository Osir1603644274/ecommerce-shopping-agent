"""Prediction-first scorer binding for prompt-contract repair V2."""
...
from agent.evaluation import shopping_mission_authoritative_state_runner_v1 as v1_runner
...
def authenticate_run(run_dir):
    ...
    required = {
        ...
        "parentRunnerSha256", "runnerSha256", "stateSchemaSha256", "predictionSchemaSha256",
        "preregistrationSha256", "casesSha256", "receiptsSha256", "promptVersion", "compilerType",
        "modelRepairCalls", "toolCalls",
    }
    if type(manifest) is not dict or set(manifest) != required:
        raise AuthoritativeScoreError("manifest contract mismatch")
    if (
        manifest["schemaVersion"] != "shopping-mission-authoritative-state-run-manifest-v2"
        or manifest["datasetId"] != base_runner.DATASET_ID or manifest["profile"] != runner.PROFILE
        or manifest["endpointOrigin"] != "https://api.deepseek.com" or manifest["temperature"] != 0
        or manifest["promptVersion"] != "ENUMS_AND_SHAPE_EXPLICIT_V2"
        or manifest["compilerType"] != "DETERMINISTIC"
        or manifest["modelRepairCalls"] != 0 or manifest["toolCalls"] != 0
    ):
    ...
    identities = {
        ...
        "parentRunnerSha256": base_runner._sha256_path(Path(v1_runner.__file__).resolve()),
...
def _translated_score(manifest, cases, receipts):
    ...
    translated_manifest = {
        ...
        "runnerSha256": manifest["runnerSha256"], "predictionSchemaSha256": manifest["predictionSchemaSha256"],
        "preregistrationSha256": manifest["preregistrationSha256"],
        "casesSha256": base_runner._sha256_bytes(case_bytes),
        "receiptsSha256": base_runner._sha256_bytes(receipt_bytes), "modelRepairCalls": 0, "toolCalls": 0,
    }
    temp_dir = Path(tempfile.mkdtemp(prefix="shopping-mission-authoritative-v2-score-"))
    try:
...
def score_run(run_dir):
    return _translated_score(*authenticate_run(run_dir))


def write_score_new(run_dir, report):
