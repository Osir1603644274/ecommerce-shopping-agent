"""Frozen public CTX1b versus MA2 paired pilot.
...
deadline. CTX1b keeps the investigation trace in the parent context; MA2 runs
...
AsyncOpenAI = DeepSeekToolCompatibleClient


ROOT = Path(__file__).resolve().parents[2]
DEFAULT_MANIFEST = ROOT / "evaluation" / "context-multiagent-public-pilot-v3.json"
DEFAULT_OUTPUT = (
    ROOT
    / "agent/evaluation/runs/context_multiagent_public_pilot_v3_attempt001"
)
...
FINAL_SYSTEM = """你是二手手机导购父 Agent。只依据结构化 ResearchReport、候选摘要和服务端校验的 candidateDecisionSupport 回答。
不得把标题里的游戏、拍照、芯片等宣传升级为实测事实。UNKNOWN 必须明确承认；不得引用报告外证据。
选择至多三款候选；若 candidateDecisionSupport 存在，selectedCandidateIds 第一项必须来自 bestVerifiedCandidateIds。claims 只写 ResearchReport 中确有的结论和引用。
    ...
    *,
    ...
    if arm == "MA2":
        request_envelope = bind_envelope_v2(
            ...
            candidate_scope_source_revision=scope.source_revision,
            candidate_scope_hash=sha256_json(scope_payload(scope)),
            parent_run_id=parent_run_id,
        ...
        if merge_receipt.outcome != "ACCEPTED" or tuple(merged_findings) != report.findings:
            raise RuntimeError("MA2 parent merge did not accept exactly once")
        reply_envelope = bind_envelope_v2(
            ...
            payload=research_report_projection(report),
        )
        ...
        communication = {
            "requestBindingHash": request_envelope.binding_hash,
            "replyBindingHash": reply_envelope.binding_hash,
            "requestSequence": request_envelope.sequence,
            "replySequence": reply_envelope.sequence,
        }
    ...
    if decision_support is not None:
        add_final_item(
            final_items,
            final_run,
            field="candidateDecisionSupport",
    ...
    final_payload = {
        ...
        "researchReport": research_report_projection(report),
        "candidateDecisionSupport": decision_support,
    }
    ...
    return {
        ...
        "rawChildObservationLeak": raw_observation_leaked if arm == "MA2" else None,
...
async def execute(manifest_path: Path, output_dir: Path, attempt_id: str) -> int:
    ...
    ma_success = [bool(pair["MA2"]["taskSuccess"]) for pair in research_pairs]
    ctx_tokens = [float(pair["CTX1b"].get("inputTokens", 0)) for pair in research_pairs]
    ma_tokens = [float(pair["MA2"].get("inputTokens", 0)) for pair in research_pairs]
    ctx_parent_tokens = [
        float(pair["CTX1b"].get("parentFinalEstimatedTokens", 0))
        for pair in research_pairs
    ]
    ma_parent_tokens = [
        float(pair["MA2"].get("parentFinalEstimatedTokens", 0))
        for pair in research_pairs
    ]
    ctx_latency = [float(pair["CTX1b"].get("durationMs", 0)) for pair in research_pairs]
    ma_latency = [float(pair["MA2"].get("durationMs", 0)) for pair in research_pairs]
    ctx_precision = [
        float(pair["CTX1b"].get("finalScore", {}).get("claimPrecision", 0))
        for pair in research_pairs
    ]
    ma_precision = [
        float(pair["MA2"].get("finalScore", {}).get("claimPrecision", 0))
        for pair in research_pairs
    ]
    ...
    safety_failures = sum(
        ...
        1
        for pair in research_pairs
        if pair["MA2"].get("rawChildObservationLeak") is not False
    )
    parent_reduction = (
    ...
    slo = manifest["slo"]
    slo_pass = (
        percentile(ma_latency, 0.95) <= slo["ma2ScenarioLatencyP95Ms"]
    ...
    visible_differences = [
        ...
        if pair["CTX1b"].get("finalAnswer") != pair["MA2"].get("finalAnswer")
    ...
    summary = {
        ...
        "allTaskSuccess": {
            ...
            "MA2": sum(bool(pair["MA2"]["taskSuccess"]) for pair in all_pairs),
        },
        "researchTaskSuccess": {
        ...
        "evidenceClaimPrecisionMacro": {
            ...
            "MA2": statistics.fmean(ma_precision or [0]),
        },
        "parentFinalEstimatedTokens": {
            "CTX1b_P50": percentile(ctx_parent_tokens, 0.50),
            "CTX1b_P95": percentile(ctx_parent_tokens, 0.95),
            "MA2_P50": percentile(ma_parent_tokens, 0.50),
            "MA2_P95": percentile(ma_parent_tokens, 0.95),
            "MA2ReductionRatio": parent_reduction,
        },
        ...
        "scenarioLatencyMs": {
            ...
            "MA2_P50": percentile(ma_latency, 0.50),
            "MA2_P95": percentile(ma_latency, 0.95),
        },
        ...
        "engineeringGate": {
            ...
            "ma2NotWorseOnSuccess": sum(ma_success) >= sum(ctx_success),
            "ma2AtLeastTwoAdditionalSuccesses": ma_only >= 2,
...
def main() -> int:
    ...
    parser.add_argument(
        "--attempt-id", default="context-multiagent-public-pilot-v3-attempt001"
    )
