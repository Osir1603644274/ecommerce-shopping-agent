...
async def serve(out,port):
    ...
    async with AsyncOpenAI(api_key=cfg.deepseek_api_key,base_url=cfg.deepseek_base_url,timeout=45,max_retries=0) as provider:
        ...
        async def recorded_run(message,**kwargs):
            ...
            # This bounded UI check exercises the real HTTP/SSE endpoint, not
            # provider token streaming. Buffer the answer so every model call
            # has a complete usage receipt before the response is published.
            answer_delta=kwargs.pop('on_answer_delta',None)
            result=await original(message,**kwargs)
            if answer_delta is not None:await answer_delta(result[0])
...
def main():
    ...
    json_new(out/'protocol.json',{'at':now(),'pid':os.getpid(),'url':f'http://127.0.0.1:{port}/',
        ...
        'providerTokenStreaming':False,'answerDelivery':'one buffered SSE delta plus actual completion event',
