...
from .common import HERE, V3, ROOT, RecordedClient, append, file_sha, freeze, check_freeze, json_new, now, rows, sha
...
async def run(out, data, selected_strict):
    ...
    # The frozen catalog adapter and the independent Validator must resolve
    # the same explicit synthetic-price policy/catalog. Process-local only.
    ...
    async with AsyncOpenAI(api_key=settings.deepseek_api_key, base_url=settings.deepseek_base_url,
        ...
        runtime._namespace = 'ctxv4-' + out.name
        runtime._ids = {}
        ...
        try:
            for conversation in data:
                ...
                for row in [r for r in schedule if r['conversationId'] == cid]:
                    ...
                    record = {**client.binding, 'executionOrdinal': row['executionOrdinal'], 'scheduleRowSha256': row['scheduleRowSha256'],
                    ...
                    try:
                        answer, tool_trace, messages, observed_run, summary = await asyncio.wait_for(llm.run_agent(
                    ...
                    from agent.app.agent_trace import get_trace_store
                    captured_trace = await get_trace_store().get(lane.run_id)
                    if captured_trace is not None:
                        body = captured_trace.model_dump(by_alias=True, mode='json')
                        append(out / 'trace_capture.jsonl', {'executionOrdinal': row['executionOrdinal'],
                            'runId': lane.run_id, 'traceSha256': sha(body), 'trace': body})
                    ...
                    if record['status'] != 'SUCCEEDED': raise RuntimeError('new_failed_turn_requires_diagnosis')
        except Exception as exc:
...
def main():
    ...
    tests = HERE / 'p0/runner_tests.xml'
    suites = list(ET.parse(tests).getroot().iter('testsuite'))
    assert suites and not any(int(s.get('failures', '0')) or int(s.get('errors', '0')) for s in suites)
    p2 = json.loads((V3 / 'p2/attempt003/result.json').read_text(encoding='utf-8'))
    ...
    for row in frozen['sources']:
        if row['path'].startswith('agent/app/') or row['path'] == '.env':
            assert file_sha(ROOT / row['path']) == row['sha256'], 'prior_source_drift'
    out = HERE / 'p4/recovery001'
    out.mkdir(parents=True, exist_ok=False)
    data = rows(HERE / 'p1/recovery.jsonl')
    json_new(out / 'conversations.json', data)
    protocol = json.loads((V3 / 'p4/confirm001/protocol.json').read_text(encoding='utf-8'))
    protocol.update(at=now(), pid=os.getpid(), conversations=len(data),
        userTurns=sum(len(c['turns']) for c in data),
        subsetDatasetSha256=file_sha(HERE / 'p1/recovery.jsonl'),
        subsetScheduleSha256=file_sha(HERE / 'p1/schedule.json'),
        inheritedMappingSha256=file_sha(HERE / 'p1/recovery_mapping.json'),
        recoveryMode='whole_affected_conversation_new_identity',
        notFreshBlindConfirmation=True, accountErrorImmediateHalt=True)
    json_new(out / 'protocol.json', protocol)
    freeze(out / 'source_freeze.json')
    with owned_redis(out):
        asyncio.run(run(out, data, p2['selectedStrict']))
    json_new(out / 'cleanup.json', {'at': now(), 'ownedRedisContextExited': True, 'sharedServiceTouched': False})
