"""Run or score authoritative MissionState prompt-contract repair V2."""
...
async def _run(args):
    ...
    client = AsyncOpenAI(api_key=settings.deepseek_api_key, base_url=settings.deepseek_base_url, timeout=120.0, max_retries=0)
    ...
    async def call_json(phase, messages, max_tokens):
        ...
        return runner.ModelCall(
            content=content, input_tokens=int(usage.prompt_tokens if usage is not None else 0),
            output_tokens=int(usage.completion_tokens if usage is not None else 0),
        )

    try:
        return await runner.run_profile(
            run_id=args.run_id, output_dir=args.output_dir.resolve(), model=settings.deepseek_model,
            endpoint=settings.deepseek_base_url, call_json=call_json, concurrency=args.concurrency,
        )
    finally:
        await client.close()
...
def main():
    ...
    if args.command == "run":
        ...
        from agent.evaluation.shopping_mission_authoritative_state_scorer_v2 import score_run, write_score_new
