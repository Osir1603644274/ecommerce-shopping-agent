...
OLD=Path('D:/agent-datasets/behavior-search-v1');V3=Path('D:/agent-datasets/behavior-history-ablation-v3');SNAP=Path('D:/agent-datasets/behavior-history-v2');OUT=Path('D:/agent-datasets/behavior-interest-v5')
...
def build():
 ...
 meta=np.load(OLD/'item_meta.npy',mmap_mode='r');vocab=[set(),set()];frequency=[collections.Counter(),collections.Counter()]
 with (OLD/'train.jsonl').open(encoding='utf8') as rf,(SNAP/'train_snapshots.jsonl').open(encoding='utf8') as sf:
  for rl,sl in zip(rf,sf,strict=True):
   ...
   current=set(tuple(map(int,meta[i])) for i in r['impressed_item_ids'])
   for j in [0,1]:frequency[j].update({p[j] for p in current if p[j]>0})
   pairs=current|set(recent(s,meta))
   for p in pairs:
    for j in [0,1]:
     if p[j]>0:vocab[j].add(p[j])
 vocab=[{v for v in vs if frequency[j][v]>=20} for j,vs in enumerate(vocab)]
 maps=[{v:i+2 for i,v in enumerate(sorted(vs))} for vs in vocab]
 save('FREQUENCY_RULE.json',{'min_distinct_train_candidate_requests':20,'counts':frequency,'labels_used':False})
