...
PARENT = BASE.parent / "shopping_memory_extraction_v15_20260830" / "attempt001"
...
def load_manifest() -> dict[str, Any]:
    ...
    if raw["schemaVersion"] != "shopping-memory-extraction-recovery-manifest-v1":
...
async def run() -> int:
    ...
    if parent_report.get("decision") != "HOLD_LLM_EXTRACTION":
    ...
    os.mkdir(ATTEMPT)
    started = {
        "schemaVersion": "shopping-memory-extraction-recovery-started-v1",
    ...
    decision = (
        "BOUNDED_LLM_EXTRACTION_RECOVERY_ACCEPT"
        if all(gates.values()) else "HOLD_LLM_EXTRACTION_RECOVERY"
    )
    report = {
        "schemaVersion": "shopping-memory-extraction-recovery-report-v1",
    ...
    receipt = {
        "schemaVersion": "shopping-memory-extraction-recovery-receipt-v1",
    ...
    return 0 if decision == "BOUNDED_LLM_EXTRACTION_RECOVERY_ACCEPT" else 2
