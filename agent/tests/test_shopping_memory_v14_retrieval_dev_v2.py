...
def build_index(path: Path, categories: Path) -> None:
    ...
    connection.commit()
    connection.close()
    connection = sqlite3.connect(categories)
    connection.executescript("""
        CREATE TABLE product_category(category_id TEXT NOT NULL,product_rowid INTEGER NOT NULL,PRIMARY KEY(category_id,product_rowid)) WITHOUT ROWID;
        INSERT INTO product_category VALUES('snacks',1);
        INSERT INTO product_category VALUES('kitchenware',2);
    """)
    connection.commit()
    connection.close()
...
def test_read_only_fts_returns_stable_target_and_metrics(tmp_path: Path):
    ...
    try:
        results = engine.search('"cassava" OR "chips"', "snacks")
        assert results[0][0] == "1"
        assert engine.search('"cassava" OR "chips"', "kitchenware") == []
