...
import numpy as np
HERE=Path(__file__).resolve().parent;O=Path('D:/agent-datasets/behavior-interest-v5');OLD=Path('D:/agent-datasets/behavior-search-v1');D=Path('F:/agent/docs/experiments/behavior-interest-v5-20260916')
...
def main():
 ...
 lines+=['','## 解释与限制','','1. 预测目标是日志曝光下的点击，未点击不是人工判定不相关。无原始展示位置，不能纠正位置偏差；没有逐点击到达时间，历史可用性仍是离线假设。','2. nDCG按请求内二元点击计算，零点击请求记0。它与相关性qrel的0.83不同，不可横向比较。','3. 相同用户可跨时间分区，目标是时间外推而非全冷启动。历史回归已反复暴露，不称盲测。','4. 约20%候选缺失或未知三级类目，历史只含品牌/类目；负结果不代表所有兴趣模型无效。','5. 分层与置信区间见ANALYSIS.json；固定种子区间不包含训练随机性，未做多重比较校正。','','## 复现','','```powershell','$env:PYTHONIOENCODING="utf-8"','$env:CUBLAS_WORKSPACE_CONFIG=":4096:8"','.venv/Scripts/python.exe experiments/behavior-interest-v5/run.py build','.venv/Scripts/python.exe experiments/behavior-interest-v5/run.py train','.venv/Scripts/python.exe experiments/behavior-interest-v5/analyze.py','```','','禁止覆盖既有build/train。原始和派生产物以MANIFEST.json记录哈希；全量从头复跑需另开版本目录。']
