...
verify_package = _load("verify_package.py", "rumr_v4_verify_package_tests")
blind_builder = _load("blind_packet_builder.py", "rumr_v4_blind_builder_tests")


class RealUserMultiturnPackageV4Tests(unittest.TestCase):
    @classmethod
    ...
    def _build(self, rows, receipt=None):
        bound_receipt = receipt if receipt is not None else verify_package.dummy_runner_receipt(rows)
        return blind_builder.build_packets(rows, self.conversations, bound_receipt)
    ...
    def test_complete_no_model_verification(self) -> None:
        ...
        self.assertEqual(result["adversarialFailClosedChecks"], 13)
        self.assertGreaterEqual(result["unicodeFieldLabelVariantsRejected"], 200)
        self.assertTrue(result["runnerOwnedReceiptValidated"])
        self.assertEqual(result["preregisteredExecutionAuthorityHashesValidated"], 5)
    ...
    def test_configuration_drift_fails_closed(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "differs from preregistered execution authority"):
            self._build(forged)
    ...
    def test_systematic_unicode_and_random_case_field_variants_fail_closed(self) -> None:
        rejected = verify_package.run_visible_label_variant_checks(blind_builder)
        self.assertGreaterEqual(rejected, len(blind_builder.FORBIDDEN_VISIBLE_KEYS) * 4)

    def test_preregistered_execution_authority_drift_fails_after_full_trace_rebind(self) -> None:
        forged = copy.deepcopy(self.outputs)
        forged[0]["trace"]["controlledWorldHash"] = verify_package.sha_text("fully-rebound-world-drift")
        self._rebind_trace(forged[0])
        with self.assertRaisesRegex(ValueError, "differs from preregistered execution authority"):
            self._build(forged)

    def test_runner_owned_receipt_cannot_bind_different_outputs(self) -> None:
        forged = copy.deepcopy(self.outputs)
        original_receipt = verify_package.dummy_runner_receipt(self.outputs)
        target = forged[0]
        self._replace_answer_and_history(forged, target, "receipt-drifted-answer")
        with self.assertRaisesRegex(ValueError, "runner receipt paired output binding mismatch"):
            self._build(forged, original_receipt)
