"""Evaluate the frozen full-catalog FTS retrieval on public development only."""
...
class FullCatalogFts:
    ...
    def search(self, query: str, *, limit: int = CANDIDATE_DEPTH) -> list[tuple[str, float]]:
        rows = self.connection.execute(
            "SELECT product_id,bm25(product_fts) AS score "
            "FROM product_fts WHERE product_fts MATCH ? "
            "ORDER BY score ASC,product_id ASC LIMIT ?",
            (query, limit),
        ).fetchall()
...
def evaluate(database_path: Path, dev_path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    engine = FullCatalogFts(database_path)
    ...
    report = {
        "schemaVersion": "shopping-memory-v14-retrieval-dev-report-v1",
        "split": "public-development",
        "decision": "PUBLIC_DEV_RETRIEVAL_ACCEPT" if all(gates.values()) else "HOLD_PUBLIC_DEV_RETRIEVAL",
        "sourceRevision": SOURCE_REVISION,
        "index": {"path": str(database_path), "sha256": INDEX_SHA256, "candidateDepth": 50},
        "counts": {
...
def materialize(
    ...
    *,
    ...
    receipt = {
        "schemaVersion": "shopping-memory-v14-retrieval-dev-receipt-v1",
...
def main() -> None:
    ...
    report, rows = evaluate(args.index, args.dev)
    print(materialize(
        args.output_dir,
        report,
        rows,
