"""Versioned Dev15 unknown-risk ordering runner identity."""
...
PROTOCOL_VERSION = "used-phone-natural-guide-dev15-production-agent-v5-unknown-risk"
...
_FIELDS.update({"PROTOCOL_VERSION": PROTOCOL_VERSION, "PRODUCTION_CODE_SCOPE_SHA256": PRODUCTION_CODE_SCOPE_SHA256})
...
def _allowed() -> frozenset[Path]:
    ...
    return frozenset({
        Path(_kernel.__file__).resolve(), Path(_v1.__file__).resolve(), Path(__file__).resolve(),
        _v1.SCHEMA_PATH.resolve(),
        (root / "agent" / "scripts" / "run_used_phone_natural_guide_dev15_v4.py").resolve(),
...
def _hashes() -> dict[str, str]:
    ...
    paths = {
        ...
        "agent/evaluation/used_phone_natural_guide_dev15_runner_v4.py": Path(__file__).resolve(),
        "agent/evaluation/schemas/used_phone_natural_guide_dev15_prediction_v1.schema.json": _v1.SCHEMA_PATH.resolve(),
        "agent/scripts/run_used_phone_natural_guide_dev15_v4.py": (root / "agent" / "scripts" / "run_used_phone_natural_guide_dev15_v4.py").resolve(),
    }
...
def _identity():
    if not _LOCK.acquire(blocking=False):
        raise PublicRunnerError("Dev15 v4 identity is already active")
...
async def run_public_agent(
    *, public_cases_path: Path, public_catalog_path: Path, run_dir: Path,
    ...
    if allow_sealed_test or resume:
        raise PublicRunnerError("Dev15 v4 is public-only and requires a fresh run")
    ...
    if selected_splits != ["dev"] or selected_ids != list(_v1.EXPECTED_CASE_IDS):
        raise PublicRunnerError("Dev15 v4 requires the complete ordered public dev set")
    with _identity():
