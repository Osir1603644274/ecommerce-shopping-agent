...
def effective_overrides(workdir):
    ...
    values.update({f'mcp_servers.{json.dumps(k)}.enabled': False for k in user.get("mcp_servers", {})})
...
def dry(attempt):
    ...
    forbidden = ["MEMORY.md", "memory_summary", "SKILL.md", "0904.md", "<skills_instructions>", "<response-annotations>"]
    leaks = [x for x in forbidden if x in text]
    write_new(out / "dry_result.json", {"status": "HOLD" if leaks else "PROMPT_EXPORT_CLEAN_PENDING_LIVE_CHECK",
...
def run(attempt):
    verify_freeze()
    out = HERE / attempt
    if read(out / "dry_result.json")["status"] != "PROMPT_EXPORT_CLEAN_PENDING_LIVE_CHECK":
    ...
    for row in schedule:
        ...
        status = "COMPLETED" if proc.returncode == 0 and answer and usage and not unexpected else "HOLD"
        check = {}
        if answer:
