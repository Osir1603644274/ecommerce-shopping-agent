"""Actual local Java + CE tool experiment; no web-controller or quality claim."""
import argparse
import asyncio
...
def main_checks(trace,model_events,expected_model,commerce):
    ...
    checks={'tool_ok':True,'positive_unique_ids':len(ids)==len(set(ids)) and all(type(i) is int and i>0 for i in ids),
        'ranked_ids_within_pool':set(ranked)<=set(ids),'no_synthetic_price':d['retrievalTrace']['syntheticPricePolicy']=='disabled',
...
async def execute(args):
    ...
    overrides={'product_cross_encoder_enabled':False,'product_cross_encoder_model_sha256':fingerprint(bound),
               'used_phone_synthetic_price_policy':'disabled','product_title_reranker_enabled':False}
    ...
    binding={'selection_sha256':args.selection_sha256,'component_model':component,'model_binding':bound,'query':'iPhone','search_category':'手机','comparison_category':'phone',
        'component_scope':'Real existing search/detail/compare tool chain; no LLM planner, no web controller, no TaskState mutation, no relevance improvement claim',
    ...
    async def provider(request):
        ...
        for i,_ in request.pairs:
            ...
            if p.get('priceStatus')!='verified' or p.get('lifecycleStatus')!='ACTIVE' or type(p.get('availableQuantity')) is not int or p['availableQuantity']<=0:
    ...
    try:
        ...
        if not seed.get('brand') or type(seed.get('snapshotPriceMinor')) is not int:raise ValueError('Real deterministic fixture seed lacks brand/price')
        cases={'plain':[],'hard-brand':[{'key':'brand','operator':'eq','value':seed['brand'],'unit':'text','priority':'hard','source':'user'}],
               'budget':[{'key':'price_minor','operator':'lte','value':seed['snapshotPriceMinor'],'unit':'CNY_MINOR','priority':'hard','source':'user'}]}
        write_once(out/'CASES.json',{'seed_product_id':seed['id'],'case_derivation':'Lowest positive ID from the first real unchanged original search; fixture constraints only, not relevance labels','cases':cases})
        selected_trace=None
        ...
        result={'status':'COMMERCE_COMPONENT_EXPERIMENT_COMPLETE','component_checks_pass':not failures and fallback_ok and comparison.ok,
