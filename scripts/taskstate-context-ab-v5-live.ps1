...
param(
    ...
    [string]$Action = "start",
    [int]$Port = 18001,
    [string]$TraceDebugKey = "taskstate-context-ab-v5-20260829"
)

$ErrorActionPreference = "Stop"
$ProjectRoot = Split-Path -Parent $PSScriptRoot
$EvidenceDir = Join-Path $ProjectRoot "agent\evaluation\runs\shopping_task_state_context_ab_v5_20260829_attempt001\services"
$StateFile = Join-Path $EvidenceDir "treatment-process.json"
$Stdout = Join-Path $EvidenceDir "treatment.stdout.log"
$Stderr = Join-Path $EvidenceDir "treatment.stderr.log"
$Module = "agent.evaluation.shopping_task_state_context_ab_v5_server:app"
...
function Invoke-Health {
    $health = Wait-Http "http://127.0.0.1:${Port}/health" 30
    $status = Invoke-RestMethod -TimeoutSec 5 `
        -Uri "http://127.0.0.1:${Port}/internal/evaluation/taskstate-context-ab-v5" `
        -Headers @{ "X-Agent-Debug-Key" = $TraceDebugKey }
    if ($status.runtime -ne "react_v1" -or $status.reactLiveEnabled -ne $true) {
        throw "unexpected treatment runtime identity"
    }
    return [ordered]@{
        status = $health.status
        experimentId = $status.experimentId
        runtime = $status.runtime
        reactLiveEnabled = $status.reactLiveEnabled
...
function Start-Treatment {
    ...
    $environment = @{
        ...
        WEB_QUERY_INTAKE_PATH = (Join-Path $EvidenceDir "treatment-web-query-intake.sqlite3")
    ...
    if ($null -ne $state) {
        ...
        if ($null -ne $process) {
            ...
            if ($command -notmatch [regex]::Escape($Module) -or $command -notmatch "--port\s+$Port") {
                throw "refusing to stop a process that does not match the V5 treatment identity"
            }
