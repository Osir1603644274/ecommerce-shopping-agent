...
$PilotRoot = Join-Path $ProjectRoot ".runtime\react-v0-live-pilot-$PilotId"
...
try {
    ...
    & $Python @RunnerArgs
    if ($LASTEXITCODE -ne 0) { throw "RUNNER: live runner failed" }

    & $Python "agent/evaluation/used_phone_harness_behavior_scorer_v1.py" `
      "--receipts" (Join-Path $RunDir "receipts.jsonl") `
      "--output" (Join-Path $RunDir "first-action-score.json")
    if ($LASTEXITCODE -ne 0) { throw "SCORER: first-action scorer failed" }

    & $Python "agent/evaluation/used_phone_react_sequence_scorer_v1.py" `
      "--receipts" (Join-Path $RunDir "receipts.jsonl") `
      "--output" (Join-Path $RunDir "sequence-score.json")
    if ($LASTEXITCODE -ne 0) { throw "SCORER: sequence scorer failed" }
    ...
    [ordered]@{
        ...
        firstActionScore = Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $RunDir "first-action-score.json") | ConvertFrom-Json
        sequenceScore = Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $RunDir "sequence-score.json") | ConvertFrom-Json
