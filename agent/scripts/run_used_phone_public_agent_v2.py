"""Audit or run the public-only used-phone v2 contract development set."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--split", action="append", choices=("dev",))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    from agent.evaluation.used_phone_public_agent_runner_v2 import (
        PublicRunnerError,
        audit_public_inputs,
        run_public_agent,
    )
    ...
    if args.audit_only:
        if args.run_dir is not None or args.resume or args.split or args.case_id:
