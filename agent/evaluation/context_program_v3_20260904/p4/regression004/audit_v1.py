...
def audit(attempt):
    ...
    with (out/'private_states.jsonl').open(encoding='utf-8') as stream:
        for line in stream:
            ...
            observed={
                ...
                'inputHashes':record['inputMessageSha256']==turn['messageSha256']==sha(turn['rawUserText']),
    ...
    audit={'at':now(),'status':'PASS_RUNNER_EVIDENCE_EDGES' if all(checks.values()) else 'FAIL',
        ...
        'inputHashConvention':'SHA256 of canonical JSON string, not SHA256 of raw UTF8; actual raw text also retained',
    ...
    json_new(out/'integrity.json',audit);print({k:v for k,v in audit.items() if k!='sourceHashes'})
