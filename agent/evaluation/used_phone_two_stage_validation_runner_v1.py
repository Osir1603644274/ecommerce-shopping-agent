"""Blind Validation10 runner for the frozen two-stage ranking v3 production scope.

This module deliberately depends only on the public runner kernel and the
two-stage prediction materializer.  It neither imports evaluator code nor
accepts authority that could select dev, test, sealed, subset, or model paths.
"""
...
VALIDATION_CASE_IDS = EXPECTED_CASE_IDS[10:20]
...
def _allowed() -> frozenset[Path]:
    ...
    return frozenset({
        ...
        Path(_dev.__file__).resolve(),
        Path(__file__).resolve(),
        SCHEMA_PATH.resolve(),
        _V2_SCHEMA_PATH.resolve(),
        (root / "agent" / "scripts" / "run_used_phone_two_stage_validation_v1.py").resolve(),
...
def _hashes() -> dict[str, str]:
    ...
    paths = {
        ...
        "agent/evaluation/used_phone_two_stage_validation_runner_v1.py": Path(__file__).resolve(),
        ...
        "agent/scripts/run_used_phone_two_stage_validation_v1.py": (
            root / "agent" / "scripts" / "run_used_phone_two_stage_validation_v1.py"
        ).resolve(),
    }
    return {
...
def _identity():
    if not _lock.acquire(blocking=False):
        raise PublicRunnerError("Validation10 runner identity is already active")
    try:
        with (
    ...
    *, public_cases_path: Path, public_catalog_path: Path, verify_java: bool = True,
    ...
    try:
        ...
        raise PublicRunnerError(
            f"Validation10 Java read-only audit unavailable: {type(exc).__name__}: {exc}"
        ) from exc
...
def _validate_selection(
    ...
    if allow_sealed_test:
        raise PublicRunnerError("Validation10 runner forbids sealed/test authority")
    selected_splits = list(splits or ["validation"])
    if selected_splits != ["validation"]:
        raise PublicRunnerError("Validation10 runner requires exactly the validation split")
    selected_ids = list(case_ids) if case_ids is not None else list(VALIDATION_CASE_IDS)
    if selected_ids != list(VALIDATION_CASE_IDS):
        raise PublicRunnerError(
            "Validation10 runner requires the complete ordered V01-V10 case set"
        )
...
async def run_public_agent(
    *, public_cases_path: Path, public_catalog_path: Path, run_dir: Path,
    ...
    if resume:
        raise PublicRunnerError("Validation10 runner requires a fresh directory; resume is forbidden")
    if client is not None:
        raise PublicRunnerError("Validation10 no-model identity rejects external clients")
    if model_name is not None and model_name != NO_MODEL_NAME:
        raise PublicRunnerError("Validation10 no-model identity rejects model overrides")
    ...
    try:
        with _identity():
            ...
            payload = b"".join(
                _kernel.canonical_json_bytes(row) for row in normalized_rows
            )
        ...
        raise PublicRunnerError(
            f"Validation10 Java read-only preflight unavailable: {type(exc).__name__}: {exc}"
        ) from exc
    return manifest
